...
internal class Download : IDownload
{
    ...
    public async Task<Stream> StartAsync(CancellationToken cancellationToken = default)
    {
        if (string.IsNullOrWhiteSpace(Package?.Urls?.FirstOrDefault()))
        {
            ...
            {
                return await _downloadService.DownloadFileTaskAsync(Url, cancellationToken);
            }
        ...
        {
            return await _downloadService.DownloadFileTaskAsync(Package, cancellationToken);
        }
        else
        {
            return await _downloadService.DownloadFileTaskAsync(Package, Url, cancellationToken);
        }
    }
